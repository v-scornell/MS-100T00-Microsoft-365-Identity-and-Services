...
**Important - PowerShell notice:** There are two versions of PowerShell that are used in the labs for this course. The older V1 PowerShell module is known as MSOnline. The new V2 PowerShell module is known as Azure Active Directory PowerShell for Graph, or AzureAD. Where applicable, Microsoft customers are encouraged to use the newer AzureAD module. However, at present, not all MSOnline functionality has been incorporated into AzureAD. While all previous PowerShell-related labs in this training course have used the AzureAD module, this lab exercise must use MSOnline since the commands necessary to update Adatum's Active Directory forest have not yet been incorporated into the AzureAD module. As such, you will begin this task by installing the MSOnline module and then connecting to the Microsoft Online Service. You'll then run the appropriate MSOnline cmdlets to update the adatum.com domain.
...
6. In the **Sign in** dialog box that appears, log in as **Holly@xxxxxZZZZZZ.onmicrosoft.com** (where xxxxxZZZZZZ is the tenant prefix provided by your lab hosting provider) with the password **Pa55w.rd**. 
